...
export function DayMusic() {
    ...
    return (
        <div className="2xl:w-[665px] sm:w-[550px] text-white smm:mx-5 rounded-3xl bg-[#171A20] overflow-hidden" >
            {
                musicDay === undefined ? <Loading /> :
                    <div>
                        <div className=" h-96 bg-center relative"
                            style={
                        ...
                        <div className="flex flex-col gap-10 px-9">
                            <div className="mt-12 flex items-center gap-4 font-gotham">
                                <img src={musicDay.artists[0].image} alt={`image from ${musicDay.artists[0].name}`} height={68} width={68} className="rounded-full" />
                                <h1 className="font-bold text-2xl leading-7">{musicDay.artists[0].name}</h1>
                            </div>
                            ...
                            <div className="font-poppins font-bold text-xl leading-8 ">
                                <span className="opacity-50">Album: </span><span className="opacity-100">{musicDay.album.name}</span><br></br>
                                <span className="opacity-50">Lançamento: </span><span className="opacity-100">{formatedDate}</span>
                            </div>
                        </div>
                    </div>
